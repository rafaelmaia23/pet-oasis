...
>
> Documento irmão, para não duplicar racional: **`docs/adr/authorization-scope-and-lifecycle.md`** (o *porquê* longo de D2/D3/D4/D5/D6', as alternativas recusadas e as invariantes de implementação). Esta seção registra as decisões da fase; o ADR detalha o modelo. Passo-a-passo e as decisões de kickoff (K1–K28) no `docs/todo.md`.
...
- **8.0** — `UserFeature.userId` → **`userRoleId`** (FK para `UserRole`, `onDelete: Cascade`) + `@@unique([userRoleId, featureId])`; `UserRole` ganhou `grantedAt` e `@@unique([userId, roleId])`. `User.features` deixou de existir como relação — overrides são alcançados por `User.roles[].features[]`.
