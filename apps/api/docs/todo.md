...
> Nenhum domínio novo: tudo que o `pet-oasis-web` precisava da API para sair do lugar, mais a dívida de deploy que já tinha derrubado a demo uma vez. 21 issues (14 planejadas em quatro rodadas de grelha; 15–20 nasceram das revisões da 07, da 15 e da 17; a 21 é o fecho), cada uma em feat-branch própria, duas delas executadas em worktrees paralelos (10 e 06). Racional em `docs/context/identity-and-sessions.md` (janela de graça, `code`s de login, timing), `docs/context/security.md` (IP, JWT, CORS, mass assignment, comprimento) e `docs/context/infrastructure.md` (rename, redes, seed, uploads, OpenSSL, domínio); o contrato para clientes em `docs/guides/integrating-with-the-api.md`; o que ficou de fora, com o motivo, em `docs/reference/backlog.md`.
...
- **A API como contrato de máquina (10.8, 10.11, 10.6):** cada recusa de login pós-senha ganhou `code` estável (`ACCOUNT_BANNED`, `PASSWORD_RESET_REQUIRED`, `EMAIL_NOT_VERIFIED`) — foi a única mudança estrutural no módulo de erro, `code` deixou de ser fixo por subclasse — com status **403 mantido** (a senha conferiu; é a conta que está recusada) e credencial errada/email desconhecido deliberadamente indistinguíveis. A allowlist de CORS passou a sair só da variável explícita: a `APP_URL` vira o front, que fala por BFF e não tem `Origin`. O guia de integração, escrito na 10.6 como estado-alvo, virou verdade issue a issue e perdeu o aviso no fecho.
...
> `.scratch/monorepo/`.
- Progresso: 1 de 13 issues fechadas.
