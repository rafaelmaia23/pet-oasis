...
- ⬜ **Herdado da 9.6/9.7:** a **autenticação opcional** já existe e `/products` **já está montado com ela** (9.7, que só tem escrita) — o `GET` é acréscimo ao router, não remontagem; e a rota entra no balde `catalog-read` do `catalogIpLimiter`, não num limiter novo.
- ⬜ **Herdado da 9.7:** o presenter do produto já tem as duas views de staff (`internal` e `cost`, escolhidas por `read:product:cost` em `productService.viewFor`) e o `flattenProduct` que achata as duas junções. Falta a view **pública** (sem custo, sem estoque exato, com disponibilidade derivada) e o ramo anônimo — estenda `productViews`, não crie presenter novo.
- ⬜ **Herdado da 9.7:** `productRepository.findProductById` já existe com o `productInclude` completo (marca, junções achatáveis e variantes ativas ordenadas por default) — a listagem reusa o mesmo include.
...
- ⬜ **Herdado da 9.7 (X9): a tabela `ProductImage` nasce aqui**, não na 9.6/9.7 — diferente de `Pet.photoPath` e `Brand.logoPath`, que são colunas órfãs esperando dono, ela ainda não existe no schema. A relação `Product.images` entra junto, na mesma migration das rotas de imagem.
...
- ⬜ Dataset fake de catálogo (marca, categoria, tag, produto, variante) coerente, para o demo não mostrar listas vazias. **Herdado da 9.7:** o molde é `tests/factories/product.factory.ts` (parse pelo schema → `productRepository.createProduct` sem audit, com `withResolvedDefault` para a variante default); `slug` e `sku` são unique global, então a idempotência é por chave estável, e o produto precisa de marca **e** de ao menos uma categoria já semeadas. **Herdado da 9.6:** marca/categoria/tag são **transacionais** (entram no truncate do `demo-reset` e já entraram no `clearDatabase`), diferente de `Breed`; a categoria precisa ser semeada **de cima para baixo** por causa da self-FK, e nome/slug são unique global, então a idempotência é por slug estável.
