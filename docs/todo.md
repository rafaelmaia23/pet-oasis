...
- ⬜ `demo-reset.ts` passa a limpar o diretório de upload **e o seed passa a repovoá-lo** — os dois juntos, nesta sessão. **Herdado da 9.10 (AA2/AA19):** limpar sem repovoar deixaria a vitrine da demo sem foto para sempre, que é exatamente o que o seed fake existe para evitar; e limpeza sem quem a exercite seria meia-funcionalidade entregue numa sessão que não a vê funcionar. Concretamente: um punhado de `.webp` pequenos versionados em `src/lib/seed/assets/`, gravados **pelo adaptador de storage** (`src/lib/storage/`) e portanto pelo mesmo pipeline do `sharp` que a API usa — nunca copiados a mão para dentro do `UPLOAD_DIR`, senão o seed produz arquivo com forma diferente do que a API produz. Vale para produto (capa), pet e marca.
