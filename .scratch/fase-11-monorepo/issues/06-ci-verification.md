...
- [x] `turbo run typecheck lint docs:check test --affected` com `TURBO_SCM_BASE` na base do PR
      ...
      um `.md` de `apps/api/docs/` mudado → `@pet-oasis/api#test` continua na lista). Logo "PR só de
      ...
      documentada no workflow e no 11.6. (No merge com a issue 07, que tirou `docs:check` do
      Turbo, ele virou step próprio do job, fora do afetado — o ADR 0197 narra a forma final.) Segundo achado (corrigido na revisão): base inexistente
...
- [x] README da raiz: badge e seção "CI". README da API: CI na "Disciplina de processo".
      ...
      `CI=true`. Porquê em `apps/api/docs/adr/0197-ci-verifica-so-afetado-services-do-job-no-lugar-do-compose.md`, indexado; backlog com o
