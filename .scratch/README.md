...
Aqui vive o trabalho **em desenho e em execução**. É versionado: **uma pasta é um esforço**,
com a spec dele e uma issue por arquivo. Uma **fase** — o capítulo do roadmap — tem um ou mais
esforços, e todos carregam o número dela.

```
.scratch/                             ← na raiz do monorepo: um tracker para API, contratos e web
├── fase-10-frontline/                ← pasta = esforço: `fase-<n>-<slug>/`, sem zero à esquerda
...
│       └── 23-…
├── fase-11-monorepo/
├── fase-12-module-depth/             ← dois esforços da Fase 12: mesmo número, vizinhos no `ls`
└── fase-12-web-auth-spine/           ← o app entra no slug quando o esforço é de um só
```

## As regras

**Uma pasta, um esforço.** Todo diretório aqui se chama `fase-<n>-<slug>/`, flat, com o número
global da fase (o de `docs/todo.md`) e sem zero à esquerda — o `ls` sai em ordem cronológica, os
esforços de uma mesma fase saem vizinhos, e a pasta casa 1:1 com a branch, que tem **o mesmo
nome dela**. Não existe pasta sem número: trabalho que não é de nenhuma fase vai para uma branch
solta a partir da `dev`, para [`docs/reference/backlog.md`](../docs/reference/backlog.md) ou para
uma issue nova num esforço aberto. Não há subpasta por app, porque um esforço atravessa apps. O `pnpm docs:check` reprova
...
teste, e é verificável sozinha. O número é local ao esforço, e a branch carrega o nome dele
(`feat/fase-12-module-depth-02-error-codes`), nunca um número global.
...
**Spec fechada é marcada, não apagada.** No fecho do esforço, a primeira linha do `spec.md`
passa a ser:
