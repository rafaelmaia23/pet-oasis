...
| `.scratch/` | O **tracker**: uma pasta por **esforço** (`fase-<n>-<slug>/`, provado pelo `docs:check`; uma fase tem um ou mais), com a spec e uma issue por arquivo | `.scratch/README.md` |
...
## ⚠️ REGRA — TDD sempre, com fluxo de branches por esforço
...
**Hierarquia de branches (git-flow por esforço), uma só para o monorepo inteiro:**
...
- **`dev` é a base de integração** e existe sempre. Toda branch de esforço sai dela.
- Uma **fase** do roadmap (ver `docs/todo.md`) é um capítulo: um número global, um título, uma entrada no `todo.md`. Ela não é branch nem PR — ela agrupa de um a n **esforços**, e fecha quando o último deles fecha.
- O **esforço** é a unidade de trabalho e de entrega: uma pasta `.scratch/fase-<n>-<slug>/` e uma branch **com o mesmo nome da pasta** (ex.: `fase-12-module-depth`), criada a partir da `dev`. O commit de **planejamento** do esforço (a spec e as issues daquela pasta) é o primeiro commit dessa branch — nunca vai direto na `dev` nem na `main`. O porquê da forma está em `docs/adr/0002-tracker-folders-are-phases.md`.
- Cada **issue** tem sua própria branch criada a partir da branch do esforço, nomeada `feat/<nome-do-esforço>-<NN>-<slug>` (ex.: `feat/fase-12-module-depth-02-error-codes`), onde `<NN>` é o número do arquivo em `.scratch/fase-<n>-<slug>/issues/`. Ao terminar (testes + `typecheck` + `lint` + `docs:check` verdes), **mergeia de volta na branch do esforço** (`--no-ff`, com a mensagem de merge padrão do Git — sem `-m`) e apaga a branch da issue.
- Ao concluir o **esforço inteiro**, abre-se um **PR** da branch dele para a `dev` e espera-se o **CI verde** (`.github/workflows/ci.yml`: typecheck, lint e testes do que o esforço afetou, `docs:check` do repo inteiro, mais o commitlint de cada commit do PR); só então a branch do esforço é mergeada na `dev` (`--no-ff`). O verde do PR é a barreira; a máquina de quem mergeia não é.
...
- Trabalho que não pertence a nenhuma fase (correção pontual, mudança de doc, ajuste de processo) também sai da `dev`, em branch própria com nome descritivo (ex.: `docs/tracker-efforts`, `fix/<slug>`), e volta pra `dev` por merge `--no-ff`. Não ganha pasta no tracker.

Resumo do fluxo: `dev` → `fase-<n>-<slug>` (o esforço) → `feat/fase-<n>-<slug>-<NN>-<titulo>` → merge na branch do esforço → (fim do esforço, PR com CI verde) merge na `dev` → (suíte verde, PR com CI verde) merge na `main` + nova `dev`.

**A numeração de fase é global ao sistema e nunca reinicia; a da issue é local ao esforço e reinicia em `01`.** Uma fase que toca API, contrato e web tem um número só, e dois esforços da mesma fase compartilham esse número — é por isso que a branch de issue carrega o nome do esforço, e não só o número. O roadmap é agrupado em **ciclos** (Ciclo 1 = fundação, Fases 1–8; Ciclo 2 = domínio pet shop, Fase 9 em diante), mas o ciclo é só agrupamento de leitura no `docs/todo.md`: a fase seguinte à 9 é a 10, não "Ciclo 2 fase 2". O `<n>` do nome da branch depende disso — dois "fase-1" em ciclos diferentes tornariam o histórico ambíguo.
...
O trabalho em execução vive em **`.scratch/fase-<n>-<slug>/`** (na raiz, único para o monorepo): **pasta = esforço**, nomeada pelo número global da fase sem zero à esquerda, com a `spec.md` e uma **issue por arquivo** em `issues/NN-<slug>.md`. Não existe pasta sem número — trabalho que não é de nenhuma fase é branch solta, backlog ou issue num esforço aberto; o `docs:check` reprova pasta fora do padrão (`docs/adr/0002-tracker-folders-are-phases.md`). O **`docs/todo.md` é o índice das fases** — estado, um ponteiro por esforço da fase aberta, e o resumo destilado de cada esforço fechado. Consulte o índice para saber onde está o trabalho; consulte as issues para saber o que fazer.

**Todo esforço, daqui em diante, nasce e corre pelo pipeline das skills:** `grill-with-docs` (a grelha fecha as decisões, uma rodada por vez) → `to-spec` (a `spec.md`) → `to-tickets` (as issues, uma fatia vertical cada) → `implement` (teste primeiro, uma feat-branch por issue, `code-review` no fim). O `.scratch/` da raiz é o tracker de todos eles.

**Forma de registro:** a fase **aberta** ocupa poucas linhas no `docs/todo.md`, com um ponteiro por esforço — o passo-a-passo vive nas issues, não ali. O esforço **fechado** é destilado num resumo de poucos bullets, no fecho do **próprio** esforço, e a fase fecha quando o último deles fecha. Essa destilação faz parte do trabalho de fecho: o *porquê* e os gotchas viram **ADR** (no app dono da decisão, ou na raiz quando é de sistema) **antes** de a fase fechar — decisão sem dono permanente não fecha. A spec **não é apagada**: ganha a linha `Status: fechada em <data> — porquê promovido a <caminhos>`, que o `pnpm docs:check` verifica. O molde das duas formas está em `docs/guides/todo-phases.md`.
...
Quando terminar um trabalho e sobrar algo pendente para uma etapa **futura**, a pendência vira **uma issue nova** em `.scratch/fase-<n>-<slug>/issues/` — **nunca** uma nota ao fim da issue que você acabou de fechar. Anotar para trás garante que, ao chegar na etapa futura, ninguém lê a nota e a pendência se perde. Regra prática: antes de escrever "fica para depois", crie o arquivo de issue que vai resolvê-la. Se a pendência não pertence a nenhum esforço planejado, ela vai para `docs/reference/backlog.md`, com o problema que resolve e o tamanho estimado.
...
O **Ciclo 2 abriu o domínio do pet shop**: a **Fase 9 está fechada** — pets (ligados a `Customer`) e catálogo completo (produto/variante, marca, categoria em árvore, tag, busca textual com tolerância a erro de digitação, upload de imagem, vitrine pública com view por feature efetiva), ainda **sem checkout**. A **Fase 10 está fechada** e não trouxe domínio novo: desbloqueou o front web e pagou a dívida de deploy. A **Fase 11 está fechada** e não trouxe domínio: transformou este repo, in-place, no monorepo `pet-oasis` — o porquê do todo em `docs/adr/0005-monorepo-in-place-pnpm-turborepo.md`, o tracker em `.scratch/fase-11-monorepo/`. A **Fase 12 está aberta** com **dois esforços**, nesta ordem: `fase-12-module-depth` (profundidade nos módulos da API e do contrato — o route entry passa a construir o handler, `ERROR_CODES` passa a tipar o erro, e os invariantes que hoje vivem em cópia ganham um dono; a pasta nasce com o planejamento) e `fase-12-web-auth-spine` (a espinha de autenticação do web, com spec e issues herdadas do `pet-oasis-web`, em `.scratch/fase-12-web-auth-spine/`). O primeiro muda o que o segundo consome, e é por isso que ele vem antes. A **Fase 13** é carrinho, pedido e pagamento — o que dá sentido pleno ao soft delete já existente (histórico de venda íntegro) —, ainda não planejada.
