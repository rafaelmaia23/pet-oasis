...
| [`docs/planning/`](../planning/) | Spec longa demais para caber em bullets. Molde: `fase-9-contexto.md`. |
...
- A fase **em execução** fica expandida (passo-a-passo, decisões de kickoff, pendências); a fase **fechada** é destilada em poucos bullets de resultado. Destilar faz parte do trabalho de fecho — o *porquê* migra antes para `docs/context/` ou para um ADR.
...
Um conjunto grande de tickets (uma fase inteira, uma spec) vira arquivo em `docs/planning/<slug>.md`,
e o `docs/todo.md` só aponta para ele.
...
- **Mapa**: `docs/planning/<esforço>-map.md`, com o corpo Notas / Decisões-até-agora / Névoa.
- **Filho**: `docs/planning/<esforço>/NN-<slug>.md`, numerado a partir de `01`, com a pergunta no corpo. Uma linha `Tipo:` registra o tipo (`research`/`prototype`/`grilling`/`task`); uma linha `Status:` registra `claimed`/`resolved`.
- **Bloqueio**: linha `Bloqueado por: NN, NN` no topo. Desbloqueado quando todos os arquivos citados estão `resolved`.
- **Fronteira**: varra `docs/planning/<esforço>/` por arquivos abertos, desbloqueados e não reclamados; vence o de menor número.
