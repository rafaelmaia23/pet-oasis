...
Sugestão: `docs/adr/` (Architecture Decision Records) na raiz do projeto — ex.: `docs/adr/0001-auth-token-revocation.md`. Se o projeto ainda não tem pasta de docs de decisão, `docs/decisoes/` também serve. O importante é ficar fora de `src/` (não é código) mas versionado no repo, perto do CONTEXT.md/CLAUDE.md para consulta.
