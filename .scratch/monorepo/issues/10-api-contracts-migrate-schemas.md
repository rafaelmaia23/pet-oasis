...
- [x] Suíte completa da API (1279) + do pacote (32) + `typecheck` + `lint` + `docs:check`
      ...
      migração.
- [x] Documentação: a decisão "Onde o contrato vive" (11.10) é o ADR
      `apps/api/docs/adr/0199-schemas-de-request-e-views-sao-codigo-do-contrato.md`, indexado em
      `apps/api/docs/adr/README.md` (escrita no antigo contexto temático de contratos e portada
      no merge com a 07, que migrou o contexto para ADRs); o `0198` (11.9) narra o "contract" do
      refactor; `CLAUDE.md` da API (organização de módulos, não-escalação, peças de identidade),
      `apps/api/docs/guides/documenting-endpoints.md` e os ADRs `0128` e `0165` dizem onde o schema vive
      agora; README e `description` do pacote listam as entradas novas.
