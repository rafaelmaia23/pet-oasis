...
- ⬜ `src/config/env.ts` + `env.example`: adicionar `SMTP_HOST`, `SMTP_PORT` (`z.coerce.number()`), `SMTP_USER`, `SMTP_PASS`, `MAIL_FROM`, `APP_URL` (base do front pra montar links). Espelhar no `env.example`.
