...
O D5 original mandava restaurar o perfil cujo `deletedAt` batesse com o do `User`, e isso
produzia um beco sem saída: ex-cliente perde o perfil em T1, tem o `User` deletado em T2, e ao
reativar não restaura (T1 ≠ T2) nem cria do zero (a linha existe) — terminando com `User` ativo
